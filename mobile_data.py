...
for page in range(pages):
	url = urllib2.urlopen("https://www.flipkart.com/search?as=off&as-show=off&otracker=start&page="+str(page+1)+"&q=mobiles&viewType=list")
	soup = BeautifulSoup(url, 'html.parser')
	for s in soup.find_all("a", "_1UoZlX"): #loops through all products in page
		...
		# print specs[2]
		attr_list = ['Model Number', 'Model Name', 'Display Size', 'Processor Type', 'Internal Storage', 'RAM', 'Primary Camera', 'Secondary Camera', 'Weight']
...
# print mobData
df = pd.DataFrame(data=mobData)
writer = pd.ExcelWriter('mobile_data1.xlsx', engine='xlsxwriter')
df.to_excel(writer, sheet_name='Sheet1')
writer.save()
